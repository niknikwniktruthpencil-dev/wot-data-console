...
def get_character_img_html(filename, max_height_pc, img_class=""):
    """キャラクター画像のHTMLタグを生成（拡張子自動判別）"""
    b64 = get_base64_of_bin_file(filename)
    if b64:
        ext = filename.split('.')[-1].lower()
        mime = "jpeg" if ext in ["jpg", "jpeg"] else "png"
        return f'<img src="data:image/{mime};base64,{b64}" class="{img_class}" style="max-height: {max_height_pc}px; object-fit: contain; filter: drop-shadow(0px 10px 15px rgba(0,0,0,0.5));">'
    return ""
...
if st.session_state['app_mode'] == "🏠 ホーム (メインメニュー)":
    ...
    char1_html = get_character_img_html("1.jpg", 220, "home-char-left")
    char2_html = get_character_img_html("3.png", 260, "home-char-right")
    top_logo_html = f'<img src="data:image/png;base64,{logo_base64}" style="width: 50px; margin-bottom: 5px; filter: drop-shadow(0px 0px 5px rgba(255,255,255,0.3));">' if logo_base64 else ''
