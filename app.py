...
if df.empty:
    st.error("エラー: 'wot_wwii_all_tanks_modules.csv' が見つかりません。")
    st.stop()
    ...
    if r_df.empty: 
        ...
        if r_df.empty:
            ...
            if "DPM" in rank_target:
                disp_cols = ['正確な車輌名', '国', 'タイプ', 'モジュール状態', sort_col, '貫通力(100m)', 'ダメージ(全体)', '装填時間(秒)']
                ...
                disp_cols = ['正確な車輌名', '国', 'タイプ', 'モジュール状態', sort_col, '貫通力(100m)', 'ダメージ(全体)', 'DPM']
