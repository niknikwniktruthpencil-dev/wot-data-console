...
def load_and_parse_data():
    ...
    df['DPM_list'] = df['詳細・モジュール生データ'].apply(lambda x: get_match_all(r'(?:分間ダメージ|交戦ダメージ) / ([\d/ \.,]+)HP', x))
    ...
    df['ダメージ_list'] = df['詳細・モジュール生データ'].apply(lambda x: get_match_all(r'(?<!分間)(?<!交戦)ダメージ / ([\d/ \.,]+)HP', x))
    ...
    df['弾薬の最大速度'] = df['詳細・モジュール生データ'].apply(lambda x: get_match(r'(?:弾薬|砲弾)の最大速度 / ([\d/ \.,]+)M', x))
    df['弾薬の最大射程'] = df['詳細・モジュール生データ'].apply(lambda x: get_match(r'(?:弾薬|砲弾)の最大射程 / ([\d/ \.,]+)M', x))
    ...
    df['HP'] = df['詳細・モジュール生データ'].apply(lambda x: get_match(r'(?:^| / )HP\s*/?\s*([\d,]+)\s*HP', x))
    ...
    df['最大前進速度'] = df['詳細・モジュール生データ'].apply(lambda x: get_match(r'(?<!砲弾の)最大速度([\d\.]+)/', x))
    df['最大後進速度'] = df['詳細・モジュール生データ'].apply(lambda x: get_match(r'(?<!砲弾の)最大速度[\d\.]+/([\d\.]+)\(', x))
